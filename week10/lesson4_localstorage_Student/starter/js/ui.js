...
function addEditDeleteButton(quoteElement, quoteId) {
  ...
  deleteButton.addEventListener('click', (e) => {
    ...
    saveQuotes() // <== เรียกใช้ saveQuotes เมื่อลบ
  })
}
...
form.addEventListener('submit', function (e) {
  ...
  if (id) {
    ...
    saveQuotes() // <== เรียกใช้ saveQuotes เมื่ออัปเดต
  } else {
    const newQuote = addQuote(content, author)
    quotes = getAllQuotes()
    addQuoteToDOM(newQuote)
    saveQuotes() // <== เรียกใช้ saveQuotes เมื่อเพิ่ม
  }
...
function loadQuotes() {
  ...
  if (storedQuotes) {
    const loaded = JSON.parse(storedQuotes)
    setQuotes(loaded)
    quotes = getAllQuotes()
  }
}
...
loadQuotes() // <== โหลดข้อมูลก่อน
renderQuotes() // <== แล้วค่อยแสดงผล